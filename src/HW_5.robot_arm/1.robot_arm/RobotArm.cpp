...
//////////////////////////
// TEAPOT CONTROLS
float BaseTransX_t = 0.5f;  // 0
float BaseTransZ_t = 0;
float BaseSpin_t = 0;
float ShoulderAng_t = 0;   // 2
float ElbowAng_t = 0;
float WristAng_t = 0;       // 3
float WristTwistAng_t = 0;
////////////////////////////
...
const char* ourObjectPath = "./snowdoll.obj";
...
// DRAWING ROBOT PARTS : 호출시 로봇 그려짐 - 적절한 위치 지정(model로),sequence에 따라
...
void DrawFingerTip(glm::mat4 model); // 왼쪽 오른쪽 각각, wrist에 대해

void DrawObject(glm::mat4 model);
bool hasTextures = false;

void myDisplay() // ***********main의 render에서 호출
{
	...
	// model: 로봇 위치 변형, translate 정도는 joint 거리를 참고

	model = glm::translate(model, glm::vec3(BaseTransX, 0.0f, BaseTransZ)); //1번 옵션
	model = glm::rotate(model, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f)); // 2번 옵션

	DrawBase(model);

	model = glm::translate(model, glm::vec3(0.0f, 0.4f, 0.0f)); // 기본 위치 조절: 1번째 arm segemnt
	model = glm::rotate(model, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f)); //3번 옵션
	DrawArmSegment(model);

	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));	 // 기본 위치 조절: 2번째 arm segemnt
	model = glm::rotate(model, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f)); // +기본 각도 조절
	DrawArmSegment(model);


	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f)); // 기본 위치 조절: wrist
	model = glm::rotate(model, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));  // 4번 옵션 
	...
	// objectXform: 티팟 위치 변형

	objectXform = glm::translate(objectXform, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	objectXform = glm::rotate(objectXform, glm::radians(BaseSpin_t), glm::vec3(0.0f, 1.0f, 0.0f));
	...
	objectXform = glm::rotate(objectXform, glm::radians(WristTwistAng_t), glm::vec3(0.0f, 1.0f, 0.0f));
	objectXform = glm::rotate(objectXform, glm::radians(-WristTwistAng), glm::vec3(0.0f, 1.0f, 0.0f));
	objectXform = glm::rotate(objectXform, glm::radians(WristAng_t), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::rotate(objectXform, glm::radians(-WristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::translate(objectXform, glm::vec3(0.0f, -0.5f, 0.0f));

	objectXform = glm::rotate(objectXform, glm::radians(ElbowAng_t), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::rotate(objectXform, glm::radians(-ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::translate(objectXform, glm::vec3(0.0f, -0.5f, 0.0f));

	objectXform = glm::rotate(objectXform, glm::radians(ShoulderAng_t), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::rotate(objectXform, glm::radians(-ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::translate(objectXform, glm::vec3(0.0f, -0.4f, 0.0f));

	objectXform = glm::translate(objectXform, glm::vec3(BaseTransX_t, 0.0f, BaseTransZ_t));
	objectXform = glm::translate(objectXform, glm::vec3(-BaseTransX, 0.0f, -BaseTransZ));

	objectXform = glm::translate(objectXform, glm::vec3(0.05f, 0.1f, 0.0f));
	objectXform = glm::rotate(objectXform, glm::radians(180.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	objectXform = glm::scale(objectXform, glm::vec3(0.0008f, 0.0008f, 0.0008f));
	DrawObject(objectXform);

	///////////////


	glm::mat4 finger = glm::translate(model, glm::vec3(0.0f, 0.2f, 0.0f)); // 기본 위치 조절: fingerbase1
	finger = glm::rotate(finger, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));  // 회전
	DrawFingerBase(finger);

	finger = glm::translate(finger, glm::vec3(0.0f, 0.35f, 0.0f)); // 기본 위치 조절: fingertip1
	finger = glm::rotate(finger, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));  // 회전
	DrawFingerTip(finger);


	finger = glm::translate(model, glm::vec3(0.0f, 0.2f, 0.0f)); // 기본 위치 조절: fingerbase2
	finger = glm::rotate(finger, glm::radians(-FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));  // 회전
	DrawFingerBase(finger);

	finger = glm::translate(finger, glm::vec3(0.0f, 0.35f, 0.0f)); // 기본 위치 조절: fingertip2
	finger = glm::rotate(finger, glm::radians(-FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));  // 회전
	DrawFingerTip(finger);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (LeftButtonDown)  //*****마우스+숫자버튼에 따라 숫자값 state 조절
	{
		switch (RobotControl)
		{
		case 0: BaseTransX += xoffset; BaseTransZ -= yoffset;
			if (TeapotControl) {
				BaseTransX_t += xoffset; BaseTransZ_t -= yoffset;
			}break;
		case 1: BaseSpin += xoffset * 180;
			if (TeapotControl) {
				BaseSpin_t += xoffset * 180;
			}break;
		case 2: ShoulderAng += yoffset * -90; ElbowAng += xoffset * 90;
			if (TeapotControl) {
				ShoulderAng_t += yoffset * -90; ElbowAng_t += xoffset * 90;
			}break;
		case 3: WristAng += yoffset * -180; WristTwistAng += xoffset * 180;
			if (TeapotControl) {
				WristAng_t += yoffset * -180; WristTwistAng_t += xoffset * 180;
			}break;
